...
3. Reconciler reads ALL plan-{cluster}.md files + audit.md + ethics-gate.md + the deadlock summary (conflict-resolution rules are inline in this protocol — this audit-only repo has no separate conflict-resolution contract file)
...
The suffixed variants share the same format as their unsuffixed parents — differing only in filename. The canonical plan/review/build formats ship with those phases' workflows; this audit-only repo does not yet include the plan, review, or build workflow files.
