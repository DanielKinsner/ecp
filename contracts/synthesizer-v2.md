...
- **ELEMENT**: combine the cluster emission's element data — CSS selector or tag, baton e_index reference, and pixel coordinates. Example: `` `div.price-box span.price-old-live` at e5 (y=403, height=42 CSS px) ``. If the element doesn't exist (`baton_index='absent'`), say `(absent — proposed location: …)`.
