...
| 2026-07-08 | 1.2 | §4.2 giant-exact-rectangle definition refined: "giant" now means an exact-element rect that is BOTH wider than `DEFAULT_GIANT_WIDTH_PCT` AND taller than `DEFAULT_GIANT_HEIGHT_PCT` (was width **or** height). A full-width but short strip (nav bar, footer, CTA row, price strip) is a precise anchor, not a parent container, so it renders as a solid exact box instead of an approximate dashed proxy. One shared predicate `assembly/visual_quality.is_giant_exact_rect` owns the logic; the G6 down-rank (`report/v2_markers`), the `giant_exact_rectangles` gate, and the `placement_audit` weak-scorer all call it (`placement_repair._is_oversized` keeps its own full-bleed-band logic — it screens re-anchor *targets*, a different purpose). | The OR rule demoted ~80% of full-width strips to approximate dashed boxes even though they were already anchored to the correct element — the dominant "reads-as-approximate" defect. Measured across 14 local engagements: 141 of 175 giant-flagged markers were thin strips (median 4.9% viewport area, i.e. not containers). "Both dimensions large" is the honest signal for a misleading parent-container box; a thin full-width strip clearly frames its own band. |
