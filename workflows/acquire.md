...
> **Pre-capture reveal (Shopify Dawn & friends).** Before the first screenshot, `acquire_url.py` fires scroll-trigger reveal animations (`.scroll-trigger .animate--*`, which start `opacity:0` until an IntersectionObserver fires) and eager-loads lazy images/`data-src` media, then nudges the IntersectionObservers and waits ~1s. Without this, a cold capture at scroll-y=0 catches the hero **unpainted** — it renders as a black void and its heading/image are filtered as not-visible, producing false "empty hero / no headline / no media" findings (the awdmods 2026-06-08 cascade). If a page still captures with a flat/empty above-fold, treat the above-fold findings as suspect and re-capture.

> **Implementation note (`scripts/acquire_url.py`).** The script captures a flat **v1-shape** baton and then auto-converts it to the v2 shape above *in place* (`baton{,-mobile}.json` become v2; the raw v1 is preserved as `baton{,-mobile}.v1raw.json`) — so the on-disk baton always satisfies the v2 contract without a separate `baton_v1_to_v2.py` orchestration step (run that manually only to recover a v1 baton). The converted baton still carries its own `schema_version: 1` (the `schema/baton-v1.json` contract); that is independent of the engagement's `meta.json` `schema_version: 3` — baton schema versioning and meta schema versioning are separate axes, so a `schema_version: 1` baton inside a `schema_version: 3` engagement is correct, not a mismatch. When the audit lead has pre-created `docs/ecp/{id}/` with `meta.json` + `audit-trace.log` (SKILL phase order step 3), pass `--allow-existing`: the acquirer **merges** its quick-scan fields into the lead's `meta.json` (lead-authored fields win) instead of clobbering it.
